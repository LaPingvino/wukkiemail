...
- **NEXT**: (b) load-images toggle in EmailView (remote imgs stripped now). (c) index JMAP bodies into search worker (Matrix-only today). (d) per-bundle filter state. (e) Wally voice/video in FAB.
