...
- **Nested bundles** (user asked): spaces-within-spaces should nest. Grouping is currently flat — `primaryKey` picks one space. Needs: (1) the space hierarchy from MatrixSource (parent→child space tree; subspaces are m.space rooms that are m.space.child of a parent), (2) recursive bundle rendering (a bundle can contain sub-bundles + items), (3) `expandedBundles` already keyed by string so nesting just needs child keys. Start in matrix.ts: expose `getSpaceTree()` or annotate items with their full space ancestry.
