...
- **DONE: render incoming stickers** — `m.sticker` events were dropped by the timeline type filter (only m.room.message/encrypted passed), so stickers from Wally/Element showed nothing. Now allowed through and parsed as a small image (`msg.image.sticker`, plain mxc thumbnail or encrypted via the existing decrypt path); RoomPanel renders them inline at ≤160px with no lightbox link (`.msg-sticker`). SENDING stickers (a pack picker pulling im.ponies usage=sticker) is still TODO — bigger, needs a picker UI + m.sticker send.
- **Remaining follow-ups** (smaller, optional): sticker SENDING/picker, per-bundle read zero-unread edge (documented tradeoff), account-level widgets, SDK auto-enable sliding sync, and the pending SPACES diagnosis (awaiting user `wmSpaces()` output).
