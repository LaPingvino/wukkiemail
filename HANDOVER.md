...
- **NEXT**: (a) mailbox-as-bundle (JmapSource.listBundles mailboxes not merged into stream). (b) load-images toggle in EmailView (remote imgs stripped now). (c) index JMAP bodies into search worker (Matrix-only today). (d) per-bundle filter state. (e) Wally voice/video in FAB.
