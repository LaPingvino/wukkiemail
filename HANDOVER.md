...
- **Remaining follow-ups** (smaller, optional): per-bundle read zero-unread edge (documented tradeoff), sticker packs (im.ponies usage sticker), account-level widgets, SDK auto-enable sliding sync, and the pending SPACES diagnosis (awaiting user `wmSpaces()` output).
