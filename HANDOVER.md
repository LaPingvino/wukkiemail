...
- **NEXT — JMAP follow-ups**: (a) a mail viewer — clicking a `jmap:` item currently no-ops (renderItem onClick only matches `matrix:` ids); add an EmailView panel (Email/get full body, mark $seen). (b) compose/reply via EmailSubmission. (c) mailbox-as-bundle (JmapSource.listBundles returns mailbox bundles — not yet merged into the stream). (d) index JMAP bodies into the search worker (Matrix-only today).
