...
- **NEXT — "Other" bundle + removable/editable default bundles** (user asked): (1) add an "Other" bundle catching items whose auto-bundle has been hidden (and any that don't land in a shown bundle); render it near the bottom alongside the "Show N read items" control. (2) Let the user hide/remove default (auto) bundles — store a disabled-set in account data (extend the bundles account-data, e.g. `{ bundles, hidden: string[] }`); a hidden default bundle's items fall into "Other". (3) Re-add a hidden default bundle from "Other" (or a manager). (4) Make ALL bundles editable — default bundles get an edit affordance too (rename / convert to a manual filter / disable). Build on the existing bundle kebab + BundleSheet.
