...
- **NEXT — Wire JMAP + combined view**: login UI (sessionUrl+token sheet), multiplex MatrixSource + JmapSource behind one inbox (merge listItems/listBundles, route subscribe/triage by item-id prefix; origin-tag + accountId provenance already exist), then compose via EmailSubmission. Foundation in src/sources/jmap.ts. Note: search-index worker is Matrix-only today; JMAP items won't be in full-text search until indexed too.
