...
- **Remaining follow-ups** (smaller, optional): `?` keyboard-shortcuts legend, per-bundle read zero-unread edge (documented tradeoff), account-level widgets, SDK auto-enable sliding sync. The SPACES fix is shipped (SDK d19580eb0); awaiting user reload/`wmSpaces()` to confirm.
