...
- **NEXT — JMAP follow-ups**: (a) compose/reply via EmailSubmission (Identity/get + Email/set draft + EmailSubmission/set). (b) mailbox-as-bundle (JmapSource.listBundles returns mailbox bundles — not yet merged into the stream; could become auto bundles or a "load images" toggle in EmailView). (c) index JMAP bodies into the search worker (Matrix-only today). (d) per-bundle filter state. (e) Wally voice/video in FAB.
