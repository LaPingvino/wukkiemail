...
- **DONE: composer draft persistence** — a half-typed message survives navigating away (and no longer leaks across rooms). Per-room/per-thread draft in localStorage (`wukkiemail:draft:<roomId>[:<threadRootId>]`); saved from the composer input listener under a draftKey REF (not a stale closure), restored on open via an effect that sets state + the Material field through this panel's own `composerFieldRef` (so a thread overlay's 2nd composer isn't targeted), cleared on send. Emoji/mention insertions don't update the draft (only typed input does) — minor.
