...
- **DONE (status+mine): per-bundle filter state** — an opened bundle's task status chips + Mine toggle now drive a per-bundle override (`bundleFilters` keyed by node key), independent of the global All-view section-header state and of other bundles. `renderFilterChips`/`displayFilter` take a `scopeKey` (undefined = global section headers, node key = that bundle); status-chip counts are computed from the passed `scope` so they're correct per-bundle. Dropped the global `issueStatusCounts` memo. READ filter stays GLOBAL this round — it's applied in `visible` (drives counts + bundle visibility), unlike the pure render-time status/mine; lifting it to per-bundle needs `visible` to become read-inclusive with counts recomputed per scope (the remaining sub-task).
- **NEXT**: (d) per-bundle READ filter (see above). (e) Wally voice/video in FAB.
