...
- Status as of now: filter core + search wiring landed. **Step (a) DONE**: bundled stream is the All view — loose items (priority>=topLevel or pinned) + fold-open bundles grouped by primaryKey (space→dm→flavor). Extracted `renderItem` + `renderFilterChips` (shared by stream, opened bundles, and the flat fallback used by focused-bundle views + active search). `bundled` memo + `expandedBundles` state + `primaryKey` in App; `.bundle-row`/`.bundle-head`/`.bundle-body` CSS. TRANSITIONAL: top chip bar + sidebar still present (step c removes them); per-bundle chips still tune GLOBAL filter state (real per-bundle filters + manual bundles later). NOT yet built: (b) config bundle, (c) sidebar removal, (d) manual bundles, (e) search redesign.
